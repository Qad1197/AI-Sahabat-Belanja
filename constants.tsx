...
export interface Region {
  name: string;
  province: string;
  tier: 'Low' | 'Mid' | 'High' | 'ExtraHigh';
}

// Basis Harga Rasional per Makan (Normal) per Orang (Baseline Minimum)
// Dihitung berdasarkan GKM BPS + Proxy Pengeluaran Makanan Jadi
export const TIER_MIN_MEAL_PRICE: Record<string, number> = {
  'Low': 12000,      // Wilayah biaya rendah (Pedesaan Jateng, DIY)
  'Mid': 18500,      // Wilayah standar (Mayoritas Kabupaten di RI)
  'High': 27500,     // Kota besar / Ibu kota provinsi / Kawasan Industri
  'ExtraHigh': 49000 // Wilayah Papua, Maluku, & Kepulauan Terluar (Logistik Tinggi)
};

export const LIFESTYLE_MODIFIER: Record<string, number> = {
  'Sederhana': 0.75,
  'Normal': 1.0,
  'Mewah': 2.4
};
...
const rawData: Record<string, string[]> = {
  ...
  "Lampung": ["Kabupaten Lampung Tengah", "Kabupaten Lampung Utara", "Kabupaten Lampung Selatan", "Kabupaten Lampung Barat", "Kabupaten Lampung Timur", "Kabupaten Mesuji", "Kabupaten Pesawaran", "Kabupaten Pesisir Barat", "Kabupaten Pringsewu", "Kabupaten Tulang Bawang", "Kabupaten Tulang Bawang Barat", "Kabupaten Tanggamus", "Kabupaten Way Kanan", "Kota Bandar Lampung", "Kota Metro"],
  ...
  "DKI Jakarta": ["Kota Administrasi Jakarta Barat", "Kota Administrasi Jakarta Pusat", "Kota Administrasi Jakarta Selatan", "Kota Administrasi Jakarta Timur", "Kota Administrasi Jakarta Utara", "Kabupaten Administrasi Kepulauan Seribu"],
  ...
  "Sulawesi Tengah": ["Kabupaten Banggai", "Kabupaten Banggai Kepulauan", "Kabupaten Banggai Laut", "Kabupaten Buol", "Kabupaten Donggala", "Kabupaten Morowali", "Kabupaten Morowali Utara", "Kabupaten Parigi Moutong", "Kabupaten Poso", "Kabupaten Sigi", "Kabupaten Tojo Una-Una", "Kabupaten Toli-Toli", "Kota Palu"],
  ...
  "Sulawesi Barat": ["Kabupaten Majene", "Kabupaten Mamasa", "Kabupaten Mamuju", "Kabupaten Mamuju Tengah", "Kabupaten Mamuju Utara", "Kabupaten Polewali Mandar", "Kota Mamuju"],
  "Maluku": ["Kabupaten Buru", "Kabupaten Buru Selatan", "Kabupaten Kepulauan Aru", "Kabupaten Maluku Barat Daya", "Kabupaten Maluku Tengah", "Kabupaten Maluku Tenggara", "Kabupaten Maluku Tenggara Barat", "Kabupaten Seram Bagian Barat", "Kabupaten Seram Bagian Timur", "Kota Ambon", "Kota Tual"],
  "Maluku Utara": ["Kabupaten Halmahera Barat", "Kabupaten Halmahera Tengah", "Kabupaten Halmahera Utara", "Kabupaten Halmahera Selatan", "Kabupaten Kepulauan Sula", "Kabupaten Halmahera Timur", "Kabupaten Pulau Morotai", "Kabupaten Pulau Taliabu", "Kota Ternate", "Kota Tidore Kepulauan"],
  ...
  "Papua Barat": ["Kabupaten Fakfak", "Kabupaten Kaimana", "Kabupaten Manokwari", "Kabupaten Manokwari Selatan", "Kabupaten Maybrat", "Kabupaten Pegunungan Arfak", "Kabupaten Raja Ampat", "Kabupaten Sorong", "Kabupaten Sorong Selatan", "Kabupaten Tambrauw", "Kabupaten Teluk Bintuni", "Kabupaten Teluk Wondama"]
};

const determineTier = (name: string, province: string): 'Low' | 'Mid' | 'High' | 'ExtraHigh' => {
  const n = name.toLowerCase();
  const p = province.toLowerCase();

  // 1. Extra High: Papua, Maluku, & Remote Islands (Mentawai, Nias, Simeulue, etc)
  if (p.includes('papua') || p.includes('maluku') || n.includes('mentawai') || n.includes('nias') || n.includes('simeulue') || n.includes('kepulauan seribu') || n.includes('natuna') || n.includes('anambas')) {
    return 'ExtraHigh';
  }
  
  // 2. High: Major Cities (Kota) & Industrial Regions
  if (n.startsWith('kota') || p.includes('jakarta') || p.includes('kepulauan riau') || p.includes('kaltim') || n.includes('bekasi') || n.includes('tangerang') || n.includes('deliserdang')) {
    return 'High';
  }

  // 3. Low: Rural Central Java & DIY (BPS proxy for lowest meal price)
  if ((p.includes('jawa tengah') || p.includes('yogyakarta')) && n.startsWith('kabupaten')) {
    return 'Low';
  }

  // 4. Default: Standard Indonesian Regency (Mid)
  return 'Mid';
};

const processedRegions: Region[] = [];
for (const [province, districts] of Object.entries(rawData)) {
  for (const name of districts) {
    processedRegions.push({
      name,
      province,
      tier: determineTier(name, province)
    });
  }
}

export const INDONESIAN_REGIONS: Region[] = processedRegions.sort((a, b) => a.name.localeCompare(b.name));

export const getMinRationalPrice = (regionName: string, lifestyle: string): number => {
  const region = INDONESIAN_REGIONS.find(r => r.name === regionName) || INDONESIAN_REGIONS[0];
  const basePrice = TIER_MIN_MEAL_PRICE[region.tier];
  const modifier = LIFESTYLE_MODIFIER[lifestyle] || 1.0;
